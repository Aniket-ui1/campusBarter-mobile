...
import { onConversationUpdated, onMessagesSeen } from '../services/socketService';
...
export const ChatBadgeProvider = ({ children }: { children: ReactNode }) => {
    ...
    const refreshUnreadCount = useCallback(async () => {
        try {
            ...
            console.log('[ChatBadge] refreshUnreadCount fetched', {
                totalConversations: serverConvs.length,
                unreadCountsPlain: serverConvs.map((c) => Number(c.unreadCount ?? 0)),
                conversationsWithUnread: serverConvs.filter((c) => Number(c.unreadCount ?? 0) > 0).length,
            });
            ...
            setConversations(prev => {
                ...
                console.log('[ChatBadge] refreshUnreadCount result', {
                    totalConversations: deduped.length,
                    totalUnreadCount: newBadgeCount,
                    unreadCountsPlain: deduped.map((c) => Number(c.unreadCount ?? 0)),
                    unreadByConversation: deduped.map((c) => ({
    ...
    // Keep unread badges in sync even if a socket event is missed.
    useEffect(() => {
        if (!user?.id) return;
    ...
    useEffect(() => {
        ...
        const unsubUpdated = onConversationUpdated(({ conversationId, lastSenderId }) => {
            ...
            // If socket references a conversation not currently in state,
            // pull fresh data so tab/chat badges can appear for new threads.
            const existsLocally = conversationsRef.current.some(
                (conversation) => conversation.conversationId === conversationId
            );
            if (!existsLocally) {
                console.log('[ChatBadge] Unknown conversation from socket, forcing refresh', { conversationId });
                void refreshUnreadCount();
                return;
            }
    ...
    }, [calculateBadgeCount, refreshUnreadCount]);
